...
# См. также
- ORM фреимворки
- R2DBC (Reactive Relational Database Connectivity): В отличие от JDBC, R2DBC использует реактивные потоки (Reactive Streams) и неблокирующую модель ввода-вывода для работы с базами данных. Подходит для систем с большим количеством одновременных запросов.
- Spring JDBC: Это легкий слой абстракции поверх JDBC, который упрощает доступ к базе данных, предоставляя API более высокого уровня и обрабатывая многие общие задачи, такие как управление соединениями и обработка исключений.
 -Spring Data JDBC предлагает подход на основе репозиториев для взаимодействия с базами данных. Он является автоматическим выбором для приложений, которые ценят простоту как в домене, так и в генерации кода.
